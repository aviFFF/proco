  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        console.error('Error saving to MongoDB, falling back to in-memory storage:', mongoError);
  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        console.error('Error fetching from MongoDB, falling back to in-memory storage:', mongoError);
        isMongoConnected = false;
        inquiries = inMemoryInquiries.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
      }
    } else {
  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        console.error('Error updating in MongoDB, falling back to in-memory storage:', mongoError);
        isMongoConnected = false;
        
        // Fall back to in-memory storage
        const index = inMemoryInquiries.findIndex(inquiry => inquiry._id === id);
        if (index === -1) {
          return res.status(404).json({ success: false, message: 'Inquiry not found' });
        }
  ...
  try {
    ...
    if (isMongoConnected) {
      try {
        ...
        console.error('Error deleting from MongoDB, falling back to in-memory storage:', mongoError);
        isMongoConnected = false;
...
app.get('/api/notifications/subscribe', (req, res) => {
  ...
  console.log(`New SSE client connected. Total subscribers: ${notificationSubscribers.length}`);
  
  // Remove client when connection closes
  req.on('close', () => {
    notificationSubscribers = notificationSubscribers.filter(client => client.id !== clientId);
    console.log(`SSE client disconnected. Remaining subscribers: ${notificationSubscribers.length}`);
  });
});

// Function to send notifications to all subscribers
function notifySubscribers(notification) {
  console.log(`Sending notification to ${notificationSubscribers.length} subscribers: ${notification.type}`);
